...
function Infinity() {
  ...
  useEffect(() => {
    if (registerComplete && gameWin) {
      const infinito = JSON.parse(localStorage.getItem('infinito'));
      infinito.status.games++;
      infinito.status.wins++;
      infinito.status.streakChance++;
      if (infinito.status.streakChance > infinito.status.streak) {
        infinito.status.streak = infinito.status.streakChance;
      }
      if (currentChance === 0) {
        infinito.status.hist[0]++;
      } else if (currentChance === 1) {
        infinito.status.hist[1]++;
      } else if (currentChance === 2) {
        infinito.status.hist[2]++;
      } else if (currentChance === 3) {
        infinito.status.hist[3]++;
      } else if (currentChance === 4) {
        infinito.status.hist[4]++;
      }
      localStorage.setItem('infinito', JSON.stringify(infinito));
    } else if (registerComplete && !gameWin) {
      const infinito = JSON.parse(localStorage.getItem('infinito'));
      infinito.status.games++;
      infinito.status.streakChance = 0;
      localStorage.setItem('infinito', JSON.stringify(infinito));
    }
  }, [registerComplete]);
